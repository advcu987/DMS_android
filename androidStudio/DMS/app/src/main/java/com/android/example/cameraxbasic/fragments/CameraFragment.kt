...
//        outputDirectory = MainActivity.getOutputDirectory(requireContext())
    ...
    private fun bindCameraUseCases() {
        ...
        val imageAnalyzer = ImageAnalysis.Builder()
            ...
            // AdVo: replacing .setImageReaderMode(ImageAnalysis.ImageReaderMode.ACQUIRE_LATEST_IMAGE)
    ...
    private  fun outputsToNMSPredictions(outputs: FloatArray, imgScaleX: Float, imgScaleY: Float, ivScaleX: Float, ivScaleY: Float, startX: Float, startY: Float) : ArrayList<PredResult> {
        ...
        for (i in 0 until 25200) {
            if (outputs[i * 85 + 4] > 0.30f) {
                val x = outputs[i * 85]
                val y = outputs[i * 85 + 1]
                val w = outputs[i * 85 + 2]
                val h = outputs[i * 85 + 3]
                val left = imgScaleX * (x - w / 2)
                val top = imgScaleY * (y - h / 2)
                val right = imgScaleX * (x + w / 2)
                val bottom = imgScaleY * (y + h / 2)
                var max = outputs[i * 85 + 5]
                var cls = 0
                for (j in 0 until 85 - 5) {
                    if (outputs[i * 85 + 5 + j] > max) {
                        max = outputs[i * 85 + 5 + j]
                        cls = j
                    }
                }
                val rect = RectF(
        ...
        // AdVo: constant values for mOutputRow, mOutputCOlumn, mThreshold, mNmsLimit are taken from PrePostProcessor.java
        return nonMaxSuppression(results, 15, 0.30f);
    }
